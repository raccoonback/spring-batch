...
import study.springbatch.chapter10.domain.Customer;
...
public class StatementLineAggregator implements LineAggregator<Statement> {
    ...
    private static final String STATEMENT_DATE_LINE = String.format("Your Account Summary %78s", "Statement Period") + "%tD to %tD\n\n";
    ...
    private void formatHeader(Statement statement, StringBuilder output) {
        ...
        output.append(
                customer.getAddress1() + ADDRESS_LINE_TWO.substring(customer.getAddress1().length())
        );
